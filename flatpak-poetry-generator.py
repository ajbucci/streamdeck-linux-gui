...
EXCLUDE_LIST = ['pyside6', 'pyside6-addons', 'pyside6-essentials', 'shiboken6']
EXCLUDE_ARCH = ['aarch64', 'ppc64le', 'armv7l', 's390x', 'ppc64', 'i686']
            ...
            if release == version:
                for source in source_list:
                    if (
                        ...
                        and not any(source['filename'].endswith(f"{arch}.whl") for arch in EXCLUDE_ARCH)
    ...
    for section, packages in parsed_lockfile.items():
        if section == "package":
            for package in packages:
                if package['name'] in EXCLUDE_LIST:
                    continue
